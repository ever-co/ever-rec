...
export const getWorkspaceInviteDataAPI = (
  workspaceInviteId: string,
): Promise<IDataResponse<IWorkspaceInviteData | null>> => {
  return api.get(`/api/v1/workspace/invite/${workspaceInviteId}`);
};

export const joinWorkspaceAPI = (
  workspaceInviteId: string,
): Promise<
  IDataResponse<{ workspaceId: string; hasAlreadyJoined: boolean } | null>
> => {
  return api.post(`/api/v1/workspace/join/${workspaceInviteId}`);
};
// End of Workspace Invites
