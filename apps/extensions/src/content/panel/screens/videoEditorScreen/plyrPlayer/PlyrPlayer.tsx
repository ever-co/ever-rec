...
import { createVideoThumbnailURL } from '@/content/utilities/misc/videoChaptersHelperFunctions';
...
export const PlyrPlayer: FC<IPlyrProps> = ({ videoURL, disableOptions }) => {
  ...
  const [savedTime, saveCurrentTime] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    if (!streamSource.toLowerCase().endsWith(HttpStreamType.DASH)) return;
    ...
    const loadVideo = async () => {
      ...
      dash.initialize(video, streamSource, false);
  ...
  const onVideoReference = (reference: any) => {
    if (!reference || reference.plyr.source === null) return;

    videoRef.current = { plyr: reference.plyr };
    ...
    video.addEventListener('timeupdate', (event: any) => {
      const duration = videoRef.current && videoRef.current.plyr.duration;
    ...
    video.addEventListener('playing', () => (pausedRef.current = false));
    video.addEventListener('pause', () => (pausedRef.current = true));
  };
