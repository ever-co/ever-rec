...
const useFolderOrder = <
  T extends IWorkspaceDbFolder | IDbFolderData = IDbFolderData,
>(
  propsData: IExplorerData | IWorkspace | undefined | null,
  folderOrder: ItemOrderEnum,
  folderType?: FolderTypeEnum,
  filterTeamId?: string | null,
) => {
  const dispatch = useDispatch();
  const [folderData, setFolderData] = useState<T[] | null>(null);
  ...
  useEffect(() => {
    ...
    setFolderData([...(foldersData as T[])]);
  }, [folderOrder, propsData, filterTeamId, folderType]);
