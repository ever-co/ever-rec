...
export type ReactMediaRecorderRenderProps = {
  ...
  muteAudio: () => void;
  unMuteAudio: () => void;
  startRecording: (muteAudio?: boolean) => void;
  pauseRecording: () => void;
...
export function useReactMediaRecorder({
  ...
  const getMediaStream = useCallback(
    async (microphoneMuted = false) => {
      ...
      try {
        if (customMediaStream) {
          ...
          mediaStream.current.getAudioTracks().forEach((audioTrack) => {
            ...
            return (audioTrack.enabled = !microphoneMuted);
          });
          mediaStream.current
            .getVideoTracks()[0]
      ...
      console.log('-- useCallback --', microphoneMuted);
    },
  ...
  const startRecording = async (microphoneMuted = false) => {
    setError('NONE');
    if (!mediaStream.current) {
      await getMediaStream(microphoneMuted);
    }
    if (mediaStream.current) {
