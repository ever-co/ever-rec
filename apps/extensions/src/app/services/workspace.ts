...
import { ItemType } from '../interfaces/ItemTypes';
...
export const updateWorkspaceFolderData = async (
  ...
  if (data) {
    ...
    const workFoldersIndex = workFolders.findIndex(
      (x: IWorkspaceDbFolder) => x.id === folder.id,
    );
...
export const saveOriginalWSImageData = async (
  ...
  return saveOriginalWSImage(fullFileName, workspaceId)
