...
export default class AuthAC {
  ...
  static removeSlackUser({ user }: { user: IUser | null }): ActionType {
    return { type: REMOVE_SLACK_USER, payload: user };
  }

  static removeDropboxUser({ user }: { user: IUser | null }): ActionType {
    return { type: REMOVE_DROP_BOX_USER, payload: user };
  }

  static removeJiraUser({ user }: { user: IUser | null }): ActionType {
    return { type: REMOVE_JIRA_USER, payload: user };
  }

  static removeTrelloUser({ user }: { user: IUser | null }): ActionType {
    return { type: REMOVE_TRELLO_USER, payload: user };
  }
  static setTrelloUser({ payload }: { payload: IAtlassian }): ActionType {
