...
import {
  ...
  IVideoPayload,
  IVideoUploader,
} from '../../view.models/video.model';

/**
 * Base abstract class for implementing a chain-of-responsibility video uploader.
 * Each uploader decides if it can handle the request, otherwise passes it along the chain.
 */
export abstract class VideoUploader implements IVideoUploader {
  protected readonly providerId: AuthProviderId;
  private nextHandler: IVideoUploader | null = null;
   ...
   */
  public setNext(next: IVideoUploader): IVideoUploader {
  ...
  public async handle(
    ...
    if (await this.canHandle(request.token)) {
      const proceed = await this.process(request.video);
   ...
   * Must be implemented by concrete uploaders.
   */
  protected abstract process(request: IVideoPayload): Promise<IEditorImage>;
}
